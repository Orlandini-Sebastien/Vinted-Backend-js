...
router.get("/offers", async(req,res)=>{
    try {
        ...
        if(priceMax ||priceMin || product_name ){
            ...
            offers = await Offer.find(filters).sort(obj).limit(page*limit).skip((page-1)*limit).populate("owner");;
        } else {
            offers = await Offer.find().sort(obj).limit(page*limit).skip((page-1)*limit).populate("owner");
        }
